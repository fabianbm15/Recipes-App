...
const postRecipes = async (req, res) => {
   try {
      const { id, title, image, dishTypes, diets, summary, healthScore, instructions } = req.body;

      console.log(title);

      if (!title || !image || !dishTypes || !summary || !healthScore || !instructions) {
         return res.status(200).json({
      ...
      for (let i = 0; i <= 140; i++) {
         ...
         try {
            ...
            recipe = await RecipesExts.create({
               ...
               healthScore: data.healthScore,
               instructions: data.instructions ? data.instructions.replace(/<[^>]+>/g, "") : null,
            });
         } catch (error) {
            console.error(error);
            continue;
